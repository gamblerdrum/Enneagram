...
st.write("""
This is a 54-question Enneagram personality test.
Rate each statement from **1 to 4**:
...
for qid, text, t in st.session_state.shuffled:
    st.write(f"**Q{qid}.** {text}")
...
if st.button("Submit and see results ✨"):
    ...
    type_for_question = {qid: t for (qid, _, t) in QUESTIONS}

    for qid, answer in st.session_state.answers.items():
        scores[type_for_question[qid]] += answer
